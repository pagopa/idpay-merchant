...
public class PointOfSaleTransactionCheckServiceImpl implements PointOfSaleTransactionCheckService {
  ...
  @Override
  public boolean hasProcessedTransactions(String merchantId, String initiativeId, List<String> posIds) {
    if (posIds == null || posIds.isEmpty()) {
      return false;
    }
    ...
    for (String posId : posIds) {
      try {
        it.gov.pagopa.merchant.connector.transaction.dto.PointOfSaleTransactionsListDTO processedTrx = transactionConnector.getPointOfSaleTransactions(merchantId, initiativeId, posId, null, null, null, PageRequest.of(0, 1));

        if (processedTrx != null && processedTrx.getContent() != null && !processedTrx.getContent().isEmpty()) {
          log.info("[TRANSACTION-PROCESSED-CHECK] Found processed transactions for POS {}", posId);
          return true;
        }
      } catch (Exception e) {
        log.error("[TRANSACTION-CHECK] Failed to query transactions for POS {}: {}", posId, e.getMessage());
